...
class Graph:
    ...
    def addEdge(self, origin, destination, cost):
        ...
        edge_symmetry = Edge(origin=destination, destination=origin,
                             cost=cost)  # add the symmetric path with the same cost
...
class ACO:
    ...
    def __init__(self, graph, num_ants, alpha=1.0, beta=5.0, iterations=10, evaporationRate=0.5):
        ...
        self.alpha = alpha  # The importance of the pheromone 信息素重要程度因子
        self.beta = beta  # The importance of the heuristic information 启发式信息的重要性
        self.iterations = iterations  # The amount of iterations
        self.evaporationRate = evaporationRate  # The rate of evaporation 信息素蒸发速率
    ...
    def rotate(self):
        ...
        for it in range(self.iterations):
            ...
            for k in range(self.num_ants):
                for i in range(0, self.graph.num_vertices - 1):
                    ...
                    for city in citys_not_visited:
                        ...
                        probability = (math.pow(pheromone, self.alpha) * math.pow(1.0 / distance, self.beta)) / (
            ...
            for edge in self.graph.edges:
                ...
                novo_pheromone = (1.0 - self.evaporationRate) * self.graph.getPheromoneEdge(edge[0], edge[
                    1]) + somatorio_pheromone
...
if __name__ == "__main__":
    ...
    while (pointer_i < 52):
        pointer_j = pointer_i
        while (pointer_j < 52):
            graph.addEdge(d[str(int(pointer_i))], d[str(int(pointer_j))],
                          defineDistance(str(int(pointer_i)), str(int(pointer_j))))
            pointer_j = pointer_j + 1
        pointer_i = pointer_i + 1

    # Creates an instance of the ACO

    aco = ACO(graph=graph, num_ants=graph.num_vertices, alpha=1.0, beta=5.0, iterations=6000, evaporationRate=0.5)
    # rotate the algorithm
    aco.rotate()

'''
设置蚂蚁，并行搜索，每完成一次周游，释放信息素量（与解质量成正比），路径选择与信息素大小有关（初始信息素设置为相等），更新信息素
ACO : final distance is 
'''